...
struct BTB {
    ...
    void update_BTB(uint64_t ip, uint8_t b_type, uint64_t target, uint8_t taken, uint64_t lru_counter, int idx){
        // int idx = index(ip);
        uint64_t tag = get_tag(ip);
        int way = -1;
        for (uint32_t i = 0; i < theBTB[idx].size(); i++) {
...
void O3_CPU::update_btb(uint64_t ip, uint64_t branch_target, uint8_t taken,
  ...
  if (btb_entry == NULL) {
    ...
    int idx = btb_partition[partition].index(ip);

    if (partition != 8) {
      if (gic >= 1000000 && pif[idx] * crb[idx] < cif[idx] * prb[idx]) {
        if (idx == 0) {
          if (pif[idx + 1] * crb[idx + 1] < cif[idx + 1] * prb[idx + 1]) {
            idx++;
          }
        } else if (idx == 511) {
          if (pif[idx - 1] * crb[idx - 1] < cif[idx - 1] * prb[idx - 1]) {
          ...
          if (pif[idx + 1] * crb[idx + 1] < cif[idx + 1] * prb[idx + 1]) {
            idx++;
          } else if (pif[idx - 1] * crb[idx - 1] < cif[idx - 1] * prb[idx - 1]) {
            idx--;
          }
        }
      }
      trb[idx] += num_bits;
      tif[idx] += convert_partitionID_to_offsetBits(partition) - num_bits;
      crb[idx] += num_bits;
      cif[idx] += convert_partitionID_to_offsetBits(partition) - num_bits;
      wc[idx]++;
    }

	  btb_partition[partition].update_BTB(ip, branch_type, branch_target, taken, basic_btb_lru_counter[cpu], idx);
    ...
    int idx = btb_partition[partitionID].index(ip);
	  btb_partition[partitionID].update_BTB(ip, branch_type, branch_target, taken, basic_btb_lru_counter[cpu], idx);
	  basic_btb_lru_counter[cpu]++;
  }
