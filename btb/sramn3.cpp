...
#define SETS 512
#define WAYS 8
#define EXTRA_BITS 25
#define ORIGINAL_BTB_PARTITIONS 9
...
int partition_used[SETS];
int partition_writes[SETS][WAYS] = {{0}};
int tic = 0;
...
int convert_partitionID_to_offsetBits(int partitionID) {
  switch (partitionID)
  {
  case 1:
    return 4;
...
int get_lru_partition(int start_partitionID, uint64_t ip) {
  ...
  int extra_partition = EXTRA_BITS / partition_used[btb_partition[0].index(ip)];
  for (int i = start_partitionID + 1; i < ORIGINAL_BTB_PARTITIONS + extra_partition; i++) {
...
void O3_CPU::initialize_btb() {
  ...
  btb_partition[13].init_btb(512, 1);
  btb_partition[14].init_btb(512, 1);
}
...
BTB_outcome O3_CPU::btb_prediction(uint64_t ip, uint8_t branch_type) {
  if(tic == INTERVAL) {
    ...
    for(uint32_t i = 0; i < 512; i++) {
      ...
      for(uint32_t j = 2; j < 8; j++) {
        ...
        partition_writes[i][j] = 0;
      }
      partition_used[i] = convert_partitionID_to_offsetBits(partition);
    }
    tic = 0;
  }
  tic++;
  BTBEntry *btb_entry;
  int extra_partition = EXTRA_BITS / partition_used[btb_partition[0].index(ip)];
  for (int i = 0; i < ORIGINAL_BTB_PARTITIONS + extra_partition; i++) {
...
void O3_CPU::update_btb(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type) {
  ...
  if (btb_entry == NULL) {
    ...
    partition_writes[btb_partition[0].index(ip)][smallest_offset_partition_id]++;
