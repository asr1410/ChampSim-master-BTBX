...
struct BTB {
  ...
  int32_t findex(uint64_t ip) {
    return ((ip >> 2) & indexMask);
  }

  int32_t sindex(uint64_t ip) {
  ...
  BTBEntry *get_BTBentry(uint64_t ip) {
    ...
    for (uint32_t i = 0; i < theBTB[idx1].size(); i++) {
      if (theBTB[idx1][i].tag == tag) {
        return &(theBTB[idx1][i]);
      }
    }
    int idx2 = sindex(ip);
    for (uint32_t i = 0; i < theBTB[idx2].size(); i++) {
      if (theBTB[idx2][i].tag == tag) {
        return &(theBTB[idx2][i]);
      }
    }
    return entry;
  }
...
// get the lru value using the findex
  uint64_t get_lru_value_findex(uint64_t ip) {
    int idx = findex(ip);
    uint64_t lru_value;
    if (theBTB[idx].size() < assoc) {
      lru_value = 0;
...
// get the lru value using the sindex
  uint64_t get_lru_value_sindex(uint64_t ip) {
    int idx = sindex(ip);
    uint64_t lru_value;
    if (theBTB[idx].size() < assoc) {
      lru_value = 0;
...
// get the confidence value using the findex
  uint64_t get_conf_value_findex(uint64_t ip) {
    int idx = findex(ip);
    uint64_t conf_value;
    if (theBTB[idx].size() < assoc) {
      conf_value = 0;
    } else {
      conf_value = theBTB[idx][0].confidence;
      for (uint32_t i = 1; i < theBTB[idx].size(); i++) {
        if (theBTB[idx][i].confidence < conf_value) {
          assert(0);
        }
      }
    }
...
// get the confidence value using the sindex
  uint64_t get_conf_value_sindex(uint64_t ip) {
    int idx = sindex(ip);
    uint64_t conf_value;
    if (theBTB[idx].size() < assoc) {
      conf_value = 0;
...
int get_lru_partition(int start_partitionID, uint64_t ip) {
  ...
  uint64_t lru_value = btb_partition[start_partitionID].get_lru_value_findex(ip);
  for (int i = start_partitionID + 1; i < NUM_BTB_PARTITIONS; i++) {
    uint64_t partition_lru_value = btb_partition[i].get_lru_value_findex(ip);
...
// get the partition with the lowest confidence value using the findex
int get_conf_lru_partition_findex(int start_partitionID, uint64_t ip) {
  int lru_partition = -1;
  uint64_t lru_value = -1;
  uint64_t conf_value = -1;
  for(int i = start_partitionID; i < NUM_BTB_PARTITIONS; i++) {
    uint64_t partition_conf_value = btb_partition[i].get_conf_value_findex(ip);
    uint64_t partition_lru_value = btb_partition[i].get_lru_value_findex(ip);
    if(conf_value == -1) {
      if(partition_conf_value < 3) {
        lru_partition = i;
        lru_value = partition_lru_value;
        conf_value = partition_conf_value;
      } 
    } else {
      if(partition_conf_value < conf_value) {
        lru_partition = i;
        lru_value = partition_lru_value;
        conf_value = partition_conf_value;
      } else if(partition_conf_value == conf_value) {
        if(partition_lru_value < lru_value) {
          lru_partition = i;
          lru_value = partition_lru_value;
          conf_value = partition_conf_value;
...
// get the partition with the lowest confidence value using the sindex
int get_conf_lru_partition_sindex(int start_partitionID, uint64_t ip) {
  int lru_partition = -1;
  uint64_t lru_value = -1;
  uint64_t conf_value = -1;
  for(int i = start_partitionID; i < NUM_BTB_PARTITIONS; i++) {
    uint64_t partition_conf_value = btb_partition[i].get_conf_value_sindex(ip);
    uint64_t partition_lru_value = btb_partition[i].get_lru_value_sindex(ip);
    if(conf_value == -1) {
      if(partition_conf_value < 3) {
        lru_partition = i;
        lru_value = partition_lru_value;
...
void O3_CPU::update_btb(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type) {
  ...
  if (btb_entry == NULL) {
    ...
    if(branch_type == BRANCH_RETURN || branch_type == BRANCH_DIRECT_CALL || branch_type == BRANCH_DIRECT_JUMP) {
      ...
      int partition_1 = get_conf_lru_partition_findex(smallest_offset_partition_id, ip);
      if(partition_1 != -1) {
        assert(partition_1 < NUM_BTB_PARTITIONS);
        btb_partition[partition_1].update_BTB(ip, branch_type, branch_target, taken, basic_btb_lru_counter[cpu]);
      } else {
        int partition_2 = get_conf_lru_partition_sindex(smallest_offset_partition_id, ip);
        if(partition_2 != -1) {
          assert(partition_2 < NUM_BTB_PARTITIONS);
          btb_partition[partition_2].update_BTB(ip, branch_type, branch_target, taken, basic_btb_lru_counter[cpu]);
        } else {
          int partition_3 = get_lru_partition(smallest_offset_partition_id, ip);
          assert(partition_3 < NUM_BTB_PARTITIONS);
          btb_partition[partition_3].update_BTB(ip, branch_type, branch_target, taken, basic_btb_lru_counter[cpu]);
        }
      }
    }
